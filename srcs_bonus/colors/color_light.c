...
/*   Updated: 2022/05/04 19:13:24 by jgainza-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
double	ft_spec_light(t_vec3 norm, t_vec3 hit_point,
		t_light lgh, t_vec3 ray_dir)
{	
	t_vec3	rebound;
	t_vec3	new_vec;
	double	spec;

	rebound = ft_vec_normalize(ft_vec_sub(ray_dir, ft_vec_multiply(norm,
					2 * ft_vec_dot(ray_dir, norm))));
	new_vec = ft_vec_normalize(ft_vec_sub(lgh.coord, hit_point));
	spec = ft_vec_dot(rebound, new_vec);
	return (pow(fmax(spec, 0.0), 25) * lgh.ratio);
}
